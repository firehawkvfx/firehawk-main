...
printf "...Query AMI with commit: $TF_VAR_ami_commit_hash"
...
if [[ "$query" != "null" ]]; then
    ...
    printf "Found $ami_role with tag matching commit: TF_VAR_vault_consul_ami_id="
    printf "  $TF_VAR_vault_consul_ami_id\n"
fi
...
if [[ "$query" != "null" ]]; then
    ...
    printf "Found $ami_role with tag matching commit: TF_VAR_vault_client_ami_id="
    printf "  $TF_VAR_vault_client_ami_id\n"
fi
...
if [[ "$query" != "null" ]]; then
    ...
    printf "Found $ami_role with tag matching commit: TF_VAR_bastion_ami_id="
    printf "  $TF_VAR_bastion_ami_id\n"
fi
...
if [[ "$query" != "null" ]]; then
    ...
    printf "Found $ami_role with tag matching commit: TF_VAR_openvpn_server_ami="
    printf "  $TF_VAR_openvpn_server_ami\n"
fi
