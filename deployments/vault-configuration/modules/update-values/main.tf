...
locals {
  ...
  secret_value = { # If a present value is different to a present default, retain the vault value.  Else use the system default.
    value = contains( keys(data.vault_generic_secret.vault_map.data), "value" ) && contains( keys(data.vault_generic_secret.vault_map.data), "default" ) && lookup( data.vault_generic_secret.vault_map.data, "value", "" ) != lookup( data.vault_generic_secret.vault_map.data, "default", "") ? lookup( data.vault_generic_secret.vault_map.data, "value", "") : local.system_default["default"] 
  }
  result_map = merge( local.system_default, local.secret_value )
}
